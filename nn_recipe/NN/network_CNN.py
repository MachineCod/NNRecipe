...
from nn_recipe.NN.Layers.pooling import MaxPool2D
...
class NetworkCNN:
    ...
    def add_layers(self, layers):
        ...
        for layer in layers:
            ...
            if not (isinstance(layer, Conv2D) or isinstance(layer, MaxPool2D) or isinstance(layer, Flatten)):
                if not issubclass(layer.__class__, Layer): #or isinstance(layer, Conv2D) or isinstance(layer, MaxPool2D)):
        ...
        if self.__batch_size is not None:
            ...
            while True:
                for batch_index in range(len(Xbatches)):
                    ...
                    out, loss = self.__propagate(x_batch, y_batch.reshape(-1, 1), opt_it)
                    opt_it += 1
    ...
    def __propagate(self, X, Y, opt_it):
        ...
        for layer in reversed(self.__layers):
            if isinstance(layer, Conv2D):
                delta = layer.calc_local_grad(delta['dY'])
                self.__opt.optimize(layer, delta['dW'], iteration=opt_it)
                print("Conv2D")
            elif isinstance(layer, MaxPool2D):
                delta = layer.calc_local_grad(delta['dY'])
                print("MaxPool2D")
            elif isinstance(layer, Flatten):
                delta = layer.calc_local_grad(delta)
                print("Flatten") 
            else:
